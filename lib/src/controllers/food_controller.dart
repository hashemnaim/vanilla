...
class FoodController extends ControllerMVC {
  ...
  void addToCart(Food food, {bool reset = false}) async {
    ...
    _newCart.food = food;
    _newCart.extras = food.extras.where((element) => element.checked.toString()==element.id).toList();
  ...
  void addToFavorite(Food food) async {
    ...
    _favorite.extras = food.extras.where((Extra _extra) {
      return _extra.checked.toString()==_extra.id;
    }).toList();
  ...
  void calculateTotal() {
    ...
    food?.extras?.forEach((extra) {
      total += extra.checked.toString()==extra.id ? extra.price : 0;
    });
    total *= quantity;
    setState(() {});
  }
