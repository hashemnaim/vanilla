...
Future<Stream<Notification>> getNotifications() async {
  ...
  final String url =
      '${GlobalConfiguration().get('api_base_url')}notifications?${_apiToken}search=notifiable_id:${_user.id}&searchFields=notifiable_id:=&orderBy=created_at&sortedBy=desc&limit=10';
...
Future<Notification> markAsReadNotifications(Notification notification) async {
  ...
  final String _apiToken = 'api_token=${_user.apiToken}';
  final String url = '${GlobalConfiguration().get('api_base_url')}notifications/${notification.id}?$_apiToken';
  final client = new http.Client();
...
Future<Notification> removeNotification(Notification cart) async {
  ...
  final String _apiToken = 'api_token=${_user.apiToken}';
  final String url = '${GlobalConfiguration().get('api_base_url')}notifications/${cart.id}?$_apiToken';
  final client = new http.Client();
