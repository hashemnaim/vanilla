...
Future<Stream<Favorite>> isFavoriteFood(String foodId) async {
  ...
  final String url = '${GlobalConfiguration().get('api_base_url')}favorites/exist?${_apiToken}food_id=$foodId&user_id=${_user.id}';
  try {
    final client = new http.Client();
...
Future<Favorite> addFavorite(Favorite favorite) async {
  ...
  final String url = '${GlobalConfiguration().get('api_base_url')}favorites?$_apiToken';
  try {
    final client = new http.Client();
...
Future<Favorite> removeFavorite(Favorite favorite) async {
  ...
  final String _apiToken = 'api_token=${_user.apiToken}';
  final String url = '${GlobalConfiguration().get('api_base_url')}favorites/${favorite.id}?$_apiToken';
  try {
    final client = new http.Client();
...
Future<Review> addFoodReview(Review review, Food food) async {
  final String url = '${GlobalConfiguration().get('api_base_url')}food_reviews';
  final client = new http.Client();
